...
\Rpackage{sampSurf}. The \Rpackage{rgeos}\footnote{The \Rpackage{gpclib} package was
  ...
  the former.} package is only used for the so-called ``chainsaw'' protocol of sampling
with fixed-area plots; therefore, if you want to use this protocol, you must install the
package.  The system dependencies for both \Rpackage{rgl} and \Rpackage{rgeos} might also
be more than some people want to deal with for installation, so this is another reason to
make it optional, allowing the majority of \Rpackage{sampSurf} functionality without these
if desired.
