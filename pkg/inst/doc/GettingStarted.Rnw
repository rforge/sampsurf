...
basic functionality, so installation is optional. To look at the different
graphics in \Rpackage{sampSurf} using \Rpackage{rgl}, the \Rpackage{rasterVis} package is
now required as well, but will be installed automatically when installing \Rpackage{sampSurf}.
