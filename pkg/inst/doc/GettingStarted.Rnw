...
\usepackage{dcolumn}                    %decimal table alignments
\usepackage{booktabs}                   %better table rules
\usepackage{bbm}
\usepackage[svgnames]{xcolor}
\usepackage{url}
\usepackage{ifthen}                     %conditional stuff

\usepackage{datetime}                   %for printing time

%
% xcolor color definitions...
%
\colorlet{jhgSteelBlue}{SteelBlue!20!SlateGray}   %for svgnames
\colorlet{jhgHyperLinks}{Navy!40!DarkSlateGray}   %for svgnames

%
% good section headers in \sffamily, with color if desired...
%
\newboolean{wantColorSec}
\setboolean{wantColorSec}{true}   %set to true for color sections
\usepackage[sf,bf,medium,raggedright]{titlesec} %section headers
\ifthenelse{\boolean{wantColorSec}}  
{ %for color sectioning; see xcolor manual for formula...
\titleformat{\section}
  {\Large\sffamily\bfseries\color{jhgSteelBlue}}{\thesection}{1em}{}
\titleformat{\subsection}
  {\large\sffamily\bfseries\color{jhgSteelBlue}}{\thesubsection}{1em}{}
\titleformat{\subsubsection}
  {\normalsize\sffamily\bfseries\color{jhgSteelBlue}}{\thesubsubsection}{1em}{}
}
{} %no color

%
%page layout: allows the graphical view of LaTeX's page layout parameters...
%
...
%page style...
%
\usepackage{fancyhdr}       %new version of fancyheadings

\usepackage[utf8]{inputenx}     %inputenx updates inputenc
...
\usepackage[pdftex,
            ...
            colorlinks,
            unicode,
            %linkcolor=jhgHyperLinks,    %color for colorlinks
            citecolor=MidnightBlue, %for citations
            pagebackref,
            pageanchor,
            pdftitle={},  %use \pdfinfo to set
            pdfauthor={J.~H.~Gove},
            pdfsubject={} %use \pdfinfo to set
           ]{hyperref}
...
%
% must manually use Sweave to get the correct definitions of its
% macros...
%
...
\title{\sffamily\bfseries \textcolor{jhgSteelBlue}{Getting Started with Package \Rpackage{sampSurf}}}
...
\date{\textcolor{jhgSteelBlue}{\today\\ \footnotesize\ampmtime}}
...
\Rcode{?install.packages}. You can install the package from within \Rc\ with\dots
%
<<eval=FALSE>>=
...
installed automatically with the command given above. In addition, it `suggests' the
\Rpackage{rgl} package, which must be installed separately. The \Rpackage{rgl} package
allows one to visualize the sampling surface in three dimensions and is not necessary for
basic functionality, so installation is optional.
...
The second and better option is to install the package and then use \Rc\ to open the
vignettes from your installation. There are two steps as follows\dots
%
\begin{enumerate}
  
  \item At the \Rc\ prompt, type \Rcode{package?sampSurf} to get to the help
    page.\footnote{Note that you need to also make sure that \Rc's internal HTML help
...
\begin{itemize}
  ...
  \item \emph{InclusionZoneClass:} The combination of a \Rclass{Stem} object and an
    ...
    the inclusion zone (the former provides the log attributes, the latter the sampling
    protocol information). This vignette details examples of the use of this class and its
    constructors for different sampling methods. 
...
individually as is normal in \Rc. For example\dots
%
<<eval=FALSE>>=
class?downLog
?downLog
...
available in PDF format can be downloaded from the \Rc\ website for the project at CRAN;
e.g., \url{http://software.rc.fas.harvard.edu/mirrors/R/}, and then choose the
\Rpackage{sampSurf} link.

\section{Summary}

This has just been a short primer to get you going. I hope you find \Rpackage{sampSurf} useful.
